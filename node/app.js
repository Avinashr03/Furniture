...
app.get('/collection_data', async (req, res) => {
    try {
        ...
        const total_count = await db.collection('Fur').estimatedDocumentCount();
        ...
        const collection_data = await db.collection('Fur').find({}, { projection: { _id: 0 } }).skip(start_index).limit(chunk_size).toArray();
